...
the code agrees with itself. The Rust crates and `go/topofbook-parser`'s
`golden_test.go` assert against these in CI, the latter reading all five with
the decoder it already had, so a layout change made on one side fails on the
other. The Wireshark dissectors do not assert against them, so a drift there
is not caught.
