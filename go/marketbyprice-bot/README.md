...
It increments `malformed_deltas_total{reason}`, never `per_instrument_gaps_total`. The gap counter measures records that never reached this process; a malformed record arrived intact and is a publisher defect, and an operator who cannot tell the two apart goes looking for packet loss that is not there.
