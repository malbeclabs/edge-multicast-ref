...
| `gap` | A book-affecting message was lost, so the book may be stale. Deltas buffer until a snapshot captured at or after the loss repairs it. |
...
Transitions: a snapshot that covers the instrument's hole and commits moves any status to `ready`. Three things move `ready` to `gap` — a confirmed per-instrument sequence gap, a delta-buffer eviction, and a malformed book-affecting message (see below). An `InstrumentReset` moves any status to `awaiting-snapshot` and records a required anchor.

**Gap recovery needs a snapshot captured at or after the hole.** Each of the three demotions records the `Per-Instrument Seq` whose mutation was lost, and until a snapshot reaches it every earlier one is refused and counted as `snapshot_discarded_total{reason="stale_instrument_seq"}`. A `gap` instrument is not indiscriminately hungry for a book: a snapshot with a lower `Last Instrument Seq` is a copy of the publisher's book from *before* the loss, so committing it would return the instrument to `ready` holding a book this book engine already knows is missing a mutation — and set the tracker below the hole, so the deltas behind it park in the reorder window and walk it to a second demotion. That second one reads as `per_instrument_gaps_total`, which reports one publisher defect, or one eviction this process chose, as `mktdata` that never arrived.

What each demotion requires follows from what it lost. A sequence gap requires the seq before the record that will replay, and never merely the hole. The branch trips two ways and both reach past it: on count, `Pending` holds deltas that arrived ahead of the missing one and they are cleared without buffering; on distance, the deltas between the hole and the held record never arrived at all. Either way a snapshot at the hole commits a book that whole run was never applied to, and the replay finds the run still missing and declares a second gap — one loss counted twice. The floor is therefore the held record's seq minus one, raised further by a scan of `Pending` for any key above it, since those are dropped here too. The held record itself is excluded: it is buffered and replays, so requiring it would refuse a snapshot that is perfectly good with that record on top. A malformed record requires its *own* seq — the seqs between it and the expected one are buffered and replay, while the one the publisher consumed for the discarded message is gone. An eviction requires the highest seq in the range it discarded, because it drops a whole buffer at once and is not triggered by a record of the victim's own. A second demotion before recovery only ever raises the requirement, so it cannot waive the first one's hole.

The requirement is checked again when the shadow commits, not only when the `SnapshotBegin` arrives. A `ready` instrument that was behind opens a shadow and can be gapped while it is still building, and that shadow was captured before the hole the demotion is for; `SnapshotBegin` ran too early to see it. An `InstrumentReset` clears the requirement, because it restarts the `Per-Instrument Seq` series — a requirement against the old series would refuse every snapshot that follows — and `Anchor Seq` guards recovery from there.
...
Which is why the record is classified **on receipt**, before the sequencing decision, and not when it comes up for apply. A malformed record that arrives ahead of the expected `Per-Instrument Seq` would otherwise be held for reordering unexamined; if the hole in front of it never fills, the reorder window expires and clears the held records, and the defect leaves with them — uncounted, with the demotion reported as `per_instrument_gaps_total`. The verdict does not depend on arrival order: the message reads malformed from its own fields, and the mutation it consumed a sequence number for is lost whether or not the records before it arrive. Records buffered while an instrument is not `ready` are classified when the buffer replays, which runs the same path.

**Which counter fires depends on which process saw the bytes.** `marketbyprice-parser` refuses `Scope = 1` with `Clear Side = 2` at decode, drops the message without failing its datagram, and counts `dz_mbp_parser_malformed_total{reason="bookclear_scope_side"}`. It emits no record for it, so on the reference pipeline the book-builder never receives one: `malformed_deltas_total` stays at zero, and the `Per-Instrument Seq` the publisher consumed reads here as an ordinary hole — a `per_instrument_gap` after the reorder window, like any other missing delta. Read the parser's counter to find the defect. The book-builder's demotion covers a record that reaches it anyway: a parser that does not refuse the pairing, or a hand-fed socket. The reason labels are shared between the two counters so one dashboard can put them side by side.

A malformed message that affects no book state does **not** demote. The line is drawn at the `Per-Instrument Seq` series: only `LevelUpdate` and `BookClear` carry it, so only they can lose a mutation the publisher counted. A `LevelUpdate` whose `Action` disagrees with this book engine's book is not on that side of the line — the absolute-apply rule still produces the correct level, so it is counted as `book_divergence_total{kind}` and the instrument stays `ready`. Nor is a discarded snapshot, which loses no delta: only the shadow is dropped and the status is untouched.

## Feed-specific behavior

**The `Last Instrument Seq` discriminator.** A periodic snapshot for an already-`ready` instrument is ignored unless its `Last Instrument Seq` exceeds the instrument's own tracker — that is, unless it was captured after deltas this subscriber never applied. The same field decides whether a `gap` instrument's recovery snapshot is new enough, against the recorded hole rather than the tracker. `Anchor Seq` is deliberately *not* used for either: it is a channel-wide `mktdata` sequence that advances on every other instrument's deltas and on every heartbeat, so comparing it would rebuild every good book on every snapshot rotation, and a per-instrument hole is only expressible in that instrument's own series.

**Shadow commit.** A snapshot is built into a shadow (`PendingSnapshot`), never into the live book. On any validation failure — wrong `Snapshot ID`, wrong `Anchor Seq`, a level count that does not match `Total Levels`, or a `Last Instrument Seq` below a hole recorded while the shadow was building — only the shadow is discarded. Status and the live book are untouched. This departs from the spec's literal "discard the partial book and revert to awaiting-snapshot" for an already-`ready` instrument, because dropping a book the deltas are keeping correct costs a full round-robin cycle of availability and buys nothing.
...
**Bounded delta buffer.** Deltas for instruments that are not `ready` are buffered, ordered by `mktdata` sequence, bounded at 200,000 records per shard. On overflow the instrument holding the most buffered records is evicted wholesale and marked `gap`; it recovers like any other gap instrument, on the next snapshot captured at or past the highest `Per-Instrument Seq` the eviction discarded. Sustained overflow means the publisher's snapshot cycle period is too long for the deployment's memory budget — the cycle-period knob and the subscriber-memory knob are the same knob — which is why it is counted rather than silently absorbed.
...
| `per_instrument_gaps_total` | Per-instrument sequence gaps confirmed after the reorder window: a `Per-Instrument Seq` this process never received. A malformed record it *did* receive is counted in `malformed_deltas_total` instead. |
| `malformed_deltas_total{reason}` | Book-affecting records this book-builder received and the spec declares malformed, each demoting its instrument to `gap` immediately: `bookclear_scope_side` (`Scope = 1` with `Clear Side = 2`) or `other`. A publisher defect, not loss in transit. Zero on the reference pipeline, where the parser refuses the pairing at decode and counts `dz_mbp_parser_malformed_total{reason}`; the reason values match, so the two sit side by side on one dashboard. |
...
| `snapshot_discarded_total{reason}` | Snapshots discarded: `stale_anchor` (captured before an `InstrumentReset`), `stale_instrument_seq` (captured before the hole that gapped the instrument), `short`, `mismatch`, `other`. A `snapshot_end` with no open shadow is not a discard — it is the healthy path where a ready, current instrument declined the begin. |
...
| `deltas_discarded_total{reason}` | Deltas seen and not applied. `stale_seq` is a duplicate or late datagram — benign in bursts, but a sustained climb on one instrument with no matching applied traffic means a snapshot set the sequence tracker ahead of reality and the book is wedged. |
...
| `malformed_delta` | no | A `BookClear` the book engine rejected; nothing was applied, the sequence trackers did not advance, and the instrument is demoted to `gap`. |
