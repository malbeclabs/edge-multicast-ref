...
func schemaV1DefBody(symbol string) []byte {
	b := make([]byte, 76)
	binary.LittleEndian.PutUint32(b[0:4], 4242) // Instrument ID
	...
	binary.LittleEndian.PutUint16(b[74:76], 9)
	return b
}
...
func schemaV2DefBody(symbol string) []byte {
	...
	binary.LittleEndian.PutUint16(b[122:124], 9)
	return b
}
...
func TestParseInstrumentDefinition_BothSchemasDecodeIdentically(t *testing.T) {
	...
	if v2.InstrumentID != 4242 || v2.Symbol != "BTC-USDT" || v2.Leg1 != "BTC" || v2.Leg2 != "USDT" {
	...
	if !v2.Expiry.Equal(schemaTestExpiry) || v2.SettleType != 1 || v2.PriceBound != 2 || v2.ManifestSeq != 9 {
		t.Errorf("tail: %+v", v2)
	}
}
...
// so the reader accepts either and nothing else.
func TestParseFrameHeader_SchemaVersions(t *testing.T) {
	for _, tc := range []struct {
		...
		buf := buildFrameHeader(mbpMagic, tc.schema, 0, 1, time.Unix(1700000400, 0), 1, 0, frameHeaderSize)
...
func TestParseFrame_InstrumentDefinitionRecordSameAcrossSchemas(t *testing.T) {
	...
	frameFor := func(schema uint8, body []byte) []byte {
		...
		frame := buildFrameHeader(mbpMagic, schema, 3, 77, ts, 1, 0, uint16(frameHeaderSize+len(msg)))
		return append(frame, msg...)
	}

	v1Recs, _, err := p.ParseFrame("refdata", frameFor(schemaV1, schemaV1DefBody("BTC-USDT")))
	if err != nil {
		t.Fatalf("schema 1 frame: %v", err)
	}
	v2Recs, _, err := p.ParseFrame("refdata", frameFor(schemaV2, schemaV2DefBody("BTC-USDT")))
	if err != nil {
