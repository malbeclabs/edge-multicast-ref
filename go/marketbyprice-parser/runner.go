...
func (r *Runner) receive(ctx context.Context, port string, conn *net.UDPConn, errs chan<- error) {
	...
	for {
		...
		records, defects, perr := r.parser.ParseDatagram(port, buf[:n])
		if perr != nil {
		...
		// Per-datagram defect counts. Returned rather than accumulated on the
...
func classifyError(err error) string {
	...
	case errors.Is(err, errDatagramTooShort), errors.Is(err, errMessageTooShort), errors.Is(err, errTruncated):
		return "truncated"
	// Defensive only, and currently unreachable: ParseDatagram drops a malformed or
	// unknown-type message and counts it rather than failing the datagram, so
	// neither error reaches here. Kept so that a future datagram-level use of either
