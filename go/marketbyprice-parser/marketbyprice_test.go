...
func buildFrame(t *testing.T, channel uint8, seq uint64, ts time.Time, resetCount uint8, msgs ...[]byte) []byte {
	...
	frame := buildFrameHeader(mbpMagic, schemaV1, channel, seq, ts, uint8(len(msgs)), resetCount, uint16(total))
	for _, m := range msgs {
...
func TestParseFrame_MessageLengthBelowHeaderFloorRejected(t *testing.T) {
	for _, length := range []uint8{0, 1, 2, 3} {
		t.Run(fmt.Sprintf("length=%d", length), func(t *testing.T) {
			p := &marketByPriceParser{}
			frame := buildFrameHeader(mbpMagic, schemaV1, 0, 1, time.Unix(1700000204, 0), 1, 0, frameHeaderSize+4)
...
func TestParseFrame_MessageLengthOverrunsFrame(t *testing.T) {
	p := &marketByPriceParser{}
	frame := buildFrameHeader(mbpMagic, schemaV1, 0, 1, time.Unix(1700000205, 0), 1, 0, frameHeaderSize+4)
...
func TestParseFrame_BadMagicRejected(t *testing.T) {
	p := &marketByPriceParser{}
	frame := buildFrameHeader(0x4444, schemaV1, 0, 1, time.Unix(1700000206, 0), 0, 0, frameHeaderSize)
	if _, _, err := p.ParseFrame("mktdata", frame); err == nil {
