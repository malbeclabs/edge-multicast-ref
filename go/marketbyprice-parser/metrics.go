...
func NewMetrics(version, commit string) *Metrics {
	...
	m.MalformedMessages = prometheus.NewCounterVec(prometheus.CounterOpts{
		...
		Help: "Individual messages the spec declares malformed, dropped without failing their datagram.",
	}, []string{"reason"})
	...
	reg.MustRegister(
		...
		m.DatagramSeqGaps, m.DatagramsMissing, m.DatagramsTotal, m.SnapshotFlagMismatch, m.MalformedMessages, m.SkippedMessages,
