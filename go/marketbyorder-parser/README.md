...
A standalone multicast subscriber that decodes DoubleZero Market-by-Order (DZ-MBO v0.1.0) wire-format datagrams and writes decoded market data records to a file or Unix socket.
...
| `dz_mbo_parser_parse_errors_total{port,reason}` | counter | Datagram decode failures, by port and failure reason |
...
| `dz_mbo_parser_datagram_seq_gaps_total{port,source_ip,channel_id}` | counter | Datagram-header sequence discontinuities detected (real UDP datagram loss events), by port, publisher source IP, and channel ID |
| `dz_mbo_parser_datagrams_missing_total{port,source_ip,channel_id}` | counter | Total datagrams missing, summed across gap magnitudes in the datagram-header sequence, by port, publisher source IP, and channel ID |
| `dz_mbo_parser_datagrams_total{port,schema_version}` | counter | Successfully parsed datagrams, by port and wire Schema Version. The way to watch a publisher's v1-to-v3 cutover: `schema_version="3"` climbing while `schema_version="1"` goes flat, then to zero, is when the v1 decode path can be retired. `schema_version="2"` should never appear; a nonzero count there means a publisher is emitting a version this parser believes does not exist |
