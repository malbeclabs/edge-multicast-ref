...
- `dz_mbp_parser_malformed_total{reason}` — `reason` in
  `{message_length_underflow, bookclear_scope_side}`. The first is the
  `Message Length < 4` case, which the spec calls out because a length of `0`
  advances the frame walk by zero bytes and spins forever on one bad datagram.
  The second is `Scope = 1` with `Clear Side = 2`, which the spec declares
  malformed because one price cannot bound both sides, and requires the
  subscriber to discard and count.
