...
- **Never name the upstream venue in commit messages, PR titles, or PR bodies.** Venue names are fine inside code and comments, so grep the existing source if you need to know which ones are meant. In commit and PR prose, describe the feed as "the live feed", "the publishers", or by feed (top-of-book, market-by-price, market-by-order).
