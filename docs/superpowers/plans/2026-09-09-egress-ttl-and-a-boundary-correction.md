...
- [x] `default_ttl` and `#[serde(default = "default_ttl")]` go. `DEFAULT_TTL` stays and its doc comment gains the sentence that it is not the document's default, so that the constant's name cannot restore one. `TtlUnstated` interpolates the constant into its message rather than retyping the number, so the value the constant carries and the line an operator is told to write cannot drift apart.
...
- [x] Every in-tree document that omits `ttl` gains it. **There were none**: the test harness's `Doc` and `replay.sh` state `ttl = 1` already, and `loopback.sh` has no document at all — it drives `loopback_publisher.rs`, which composes an `EgressPolicy` in Rust and so never went through this key. The whole suite stayed green when the key became required. Two *tests* did need it — see the revert below — and no document in a dated design or plan was touched, because a fence in a dated document is a record of that day.
