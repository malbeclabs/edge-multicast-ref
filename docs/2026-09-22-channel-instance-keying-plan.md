...
- **`marketbyorder-bot`'s `snapshotRoute` is deleted, not re-keyed.** Re-keying
  it on the channel instance fixes the collapse *between* instances and leaves
  the collapse *within* one: `Snapshot ID` is monotonic per
  `(Channel ID, Instrument ID)`, so two instruments routinely sit at one value
  inside a cycle. `marketbyprice-bot`'s open-group shape is the fix and
  `go/marketbyprice-bot/coordinator.go:19-24` already says so.
...
- `go/marketbyorder-bot/`: `record.go`, `coordinator.go`, `shard.go`,
  `events_writer.go`, `snapshot_writer.go`, `metrics.go`, `coordinator_test.go`,
  `shard_test.go`, `events_writer_test.go`, `parity_test.go`, `README.md`
- `go/marketbyprice-bot/`: `record.go`, `coordinator.go`, `shard.go`,
  `dispatch.go`, `events_writer.go`, `snapshot_writer.go`, `metrics.go`,
  `coordinator_test.go`, `dispatch_test.go`, `events_writer_test.go`,
  `README.md`
...
- `pubKey` in both parsers' `runner.go`; `snapKey` and `Coordinator.snapshotRoute`
  in `go/marketbyorder-bot`; the four comments listed in task 12.
...
`Snapshot ID` validates membership and is never the key. Two consequences:
...
- [ ] **Step 1: Write the failing tests.** (a) Two instances of one
  ...
  and neither group is overwritten. (b) Within one instance, two instruments
  mid-cycle at the **same** `Snapshot ID`: each instrument's orders reach its own
  shadow. (c) A snapshot order arriving after its `snapshot_end` is dropped and
  counted, not routed — the `marketbyorder` twin of
  `TestDispatch_StrayLevelAfterSnapshotEndIsDroppedNotRouted`
  (`go/marketbyprice-bot/coordinator_test.go:461`).
- [ ] **Step 2: Run, watch (a) and (b) fail** — (a) because one `Channel ID` gives
  one route, (b) because the route is keyed on the shared `Snapshot ID`.
...
> Then key it on `{instance, snapshotID}` instead of the open group: test (b)
> must fail. Killing only the first leaves the defect
...
## Task 9: `marketbyorder-bot` — `instKey` on the channel instance

**Files:** `go/marketbyorder-bot/shard.go` + `shard_test.go`, `parity_test.go`
...
- [ ] **Step 1: Write the failing test.** Two instances of one `Channel ID`
  carrying one `Instrument ID`: two independent books, two independent
  per-instrument sequence positions, and a per-instrument gap on one raising
  `per_instrument_gaps_total` without demoting the other. Then a `Reset Count`
  change on one instance: `resetChannel` wipes that instance's instruments,
  refdata and buffered deltas and leaves the other instance's `StatusReady` book
  standing.
- [ ] **Step 2: Run, watch it fail** — one `instKey`, so the two instances share
  one `Instrument`, one sequence position and one delta buffer, and one reset
  wipes both.
- [ ] **Step 3: Re-key `instKey` and thread the instance through** `apply`,
  `handle`, `bufferDelta`, `replayBuffer`, `refdataFor` and `resetChannel`.
- [ ] **Step 4: Full suite, then `-race`**, `gofmt`, commit.

> **The mutant is the shared book.** Drop `inst` from `instKey` and the test must
> fail twice over: the interleaved deltas of two instances are applied to one
> `Instrument`, so each instance's `per_instrument_seq` reads as a gap in the
> other's, and the reset takes both books. The second half — the book that must
> *not* be wiped — is the assertion to check the mutant on most carefully,
> because a test asserting only that the right book was wiped passes against a
> `resetChannel` that wipes everything.
...
- [ ] **Step 1: Write the failing tests**, in both book-builders. (a) A
  ...
  `mktdata` port raises the counter and drops no group.
...
> **Three mutants.** Delete the `open` deletion: (a) must fail, and this is the
...
> or `seq != last+1`: (c) must fail. Test (d) is the one to distrust — it passes
> against a check that does nothing at all — so run it against the deleted-check
> mutant and confirm it is (a) and not (d) that fails.

---

## Task 11: `marketbyprice-bot` — `resetCount`, `open` and `instKey` on the instance

**Files:** `go/marketbyprice-bot/coordinator.go` + `coordinator_test.go`,
`go/marketbyprice-bot/shard.go`, `dispatch.go` + `dispatch_test.go`
...
- [ ] **Step 1: Write the failing tests**, the twins of tasks 7, 8(a) and 9:
  ...
  reset on one instance sparing the other's book; each instance's snapshot levels
  stamped with its own group's instrument. Extend
  ...
  (`:493`).
- [ ] **Step 2: Run, watch them fail.**
- [ ] **Step 3: Re-key all three**, and narrow `resetChannel`'s five keyed loops
  (`dispatch.go:338-374`) to the instance.
- [ ] **Step 4: Full suite, then `-race`**, `gofmt`, commit.

> **The mutant is the same one three times**, and each must be killed
> independently: `rec.ChannelID` back in `resetCount` fails the barrier count;
> back in `open` fails the stamping test with one instance's levels carrying the
> other's instrument; back in `instKey` fails the spared-book assertion. Run all
> three reverts separately — a single combined revert can fail one test and
> obscure that another asserts nothing.
...
`level_snapshots` rows take them from the instance its `instKey` now carries.
...
- [ ] **Step 1: Write the failing tests.** Per table, the row map carries both
  keys with the record's values. And: a record with a zero `netip.Addr` raises
  the counter, a record with `dst_port` 0 raises it, and a fully stamped record
  does not.
- [ ] **Step 2: Run, watch them fail.**
- [ ] **Step 3: Add the keys and the counter.**
- [ ] **Step 4: Full suite in both**, `gofmt`, commit.

> **Two mutants.** Drop either key from any one row map and that table's test must
> fail. Then — the one that matters — make the counter increment unconditionally,
> or never: the fully-stamped case must fail on the first, the zero cases on the
...
"Sequence gaps (per-instrument)", of both dashboards. Without it `lagInFrame`
...
- [ ] **Step 2: Edit both dashboard `rawSql` strings.**
...
- `SELECT DISTINCT source_addr, dst_port, channel_id FROM marketbyorder.events`
  returns two rows for one `channel_id`.
